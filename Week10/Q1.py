...
# COMP2152 — Devkumar Dipakbhai Patel
# ============================================================
...
def get_all_credentials():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, website, username, password FROM vault ORDER BY website ASC"
    )
...
def find_credential(website):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, website, username, password FROM vault WHERE website = ?",
        (website,)
    )
    rows = cursor.fetchall()
    conn.close()
...
if _name_ == "_main_":
    ...
    for site, user, pw in credentials:
        ...
        print(f" Saved: {site}" + (f" ({user.split('')[0]})" if "" in user else ""))
